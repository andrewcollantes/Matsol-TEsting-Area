...
function openReportPopup(record, index, updateIndex) {
    ...
    // Close the edit modal immediately - the first step has only staged changes.
    ...
    const refNoEl          = document.getElementById('report-refno');
...
if (reportForm) {
    reportForm.addEventListener('submit', async (e) => {
        ...
        const pendingMachineUpdate = window.pendingMachineUpdate;
        if (!pendingMachineUpdate || pendingMachineUpdate.recordIndex !== index) {
            showToast('The pending update expired. Please start again.', 'warning');
            closeReportModal();
            return;
        }
        ...
        const refNo          = refNoEl          ? refNoEl.value.trim()          : '';
        ...
        if (!problem || !action || !recommendation || !refNo) {
            showToast('Problem, Action, Recommendation, and Reference No fields are required.', 'warning');
            return;
        }

        const todayStr = getTodayDateString();

        const nextUpdateIndex = Array.isArray(record.updates) ? record.updates.length : 0;
        ...
        const updateEntry = {
            date: todayStr,
            activityType: pendingMachineUpdate.activityType,
            submittedBy: typeof CURRENT_USER_FULLNAME !== 'undefined' ? CURRENT_USER_FULLNAME : 'Unknown User',
            status: pendingMachineUpdate.status,
            runningHours: pendingMachineUpdate.runningHours,
            description: pendingMachineUpdate.description,
            maintenanceUpdated: pendingMachineUpdate.maintenanceUpdated,
            maintenanceServiceDate: pendingMachineUpdate.maintenanceServiceDate,
            partsUpdated: pendingMachineUpdate.partsUpdated,
            partServiceDates: pendingMachineUpdate.partServiceDates,
            partServiceHours: pendingMachineUpdate.partServiceHours
        };

        const nextUpdates = Array.isArray(record.updates)
            ? [...record.updates, updateEntry]
            : [updateEntry];
        ...
        try {
            const response = await fetch(`/client/${encodeURIComponent(CLIENT_ID)}/machines/update`, {
                ...
                body: JSON.stringify({
                    ...
                    partServiceHours: pendingMachineUpdate.partServiceHours,
                    updates:        nextUpdates,
                    report:         { ...reportEntry, updateIndex: nextUpdateIndex },
                    updateIndex:    nextUpdateIndex
                })
            });
            ...
            showToast(error.message || 'Failed to save report.', 'warning');
        }
    });
}
