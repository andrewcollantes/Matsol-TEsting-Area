...
function getMonitoringActivityDetail(row) {
    ...
    if (activityType === 'pull_out') {
        return 'Entire unit pulled out';
    }

    if (activityType === 'cannibalize') {
        return getRowDataText(row, 'partsCsv') || getCellText(row, 6) || 'Selected parts for cannibalize';
    }
...
function openKpiDetail(kpiKey) {
    ...
    else if (kpiKey === 'unitsInstalled') {
        const total = d.totalUnits || 0;
        html += `<div style="display:flex;align-items:center;gap:16px;">
            <div style="flex:0 0 auto;padding:18px;background:#f8fafc;border-radius:12px;border:1px solid #e6eef9;text-align:center;">
                <strong style="font-size:24px;color:var(--navy);">${total}</strong>
                <div style="font-size:12px;color:var(--muted);margin-top:6px;">Units Installed</div>
            </div>
            <div style="flex:1 1 auto;color:var(--text);">
                <p style="margin:0 0 8px 0;color:var(--muted)">Total installed units across all clients (ongoing and completed).</p>
                ${Array.isArray(d.clientHighlights) && d.clientHighlights.length ? '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">' + d.clientHighlights.slice(0,6).map(c => `<div style="padding:8px;background:#fff;border:1px solid #eef2f7;border-radius:8px;"><strong style="display:block;color:var(--navy)">${escapeHtml(c.clientName)}</strong><small style="color:var(--muted)">${c.units} units</small></div>`).join('') + '</div>' : ''}
            </div>
        </div>`;
    }
...
function onMonitorRangeChange(range) {
    ...
    .then(data => {
        ...
        updateKpiNum('kpi-unitsInstalled', (m.totalUnits || 0));
