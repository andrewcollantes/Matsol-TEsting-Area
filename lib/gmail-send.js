...
/**
 * Resolve Gmail API OAuth2 settings.
 *
 * @returns {{ user: string, clientId: string, clientSecret: string, refreshToken: string } | null}
 */
function resolveGmailApiAuth() {
...
/**
 * Send a single transactional email through Google Gmail REST API (over HTTPS).
 * Works on Render Free tier where SMTP ports are blocked.
 * @param {{ to: string, subject: string, text: string, html?: string }} opts
 * @returns {Promise<{ sent: boolean, skipped: boolean, error?: string, provider?: string }>}
 */
async function sendGmailApiLink(opts) {
  ...
  const authConf = resolveGmailApiAuth();
  if (!authConf) {
    return { sent: false, skipped: true };
  }
  ...
  try {
    ...
    oauth2Client.setCredentials({
      refresh_token: authConf.refreshToken
    });
    ...
    return { sent: true, skipped: false, provider: 'gmail-api' };
  } catch (err) {
    console.error('sendGmailApiLink:', err);
    return {
      sent: false,
      skipped: false,
      provider: 'gmail-api',
...
function isOutboundEmailConfigured() {
  return Boolean(resolveGmailApiAuth()) || Boolean(resolveResendApiKey()) || Boolean(resolveGmailAuth());
}

/** Short hint for admin UI after creating invite/reset */
function outboundEmailUiHint() {
  if (resolveGmailApiAuth()) {
    return ' Email is sent via Gmail API (HTTPS — works on Render/Vercel and sends to anyone).';
  }
  if (resolveGmailAuth()) {
    return ' Email is sent via Gmail SMTP (Allows sending to any email).';
  }
  if (resolveResendApiKey()) {
    return ' Email is sent via Resend API (Sandbox only sends to owner email).';
  }
  return ' (No email API/SMTP configured — copy the link above.)';
}
...
async function sendGmailAppLink(opts) {
  ...
  const hasGmailApi = Boolean(resolveGmailApiAuth());
  const hasResend = Boolean(resolveResendApiKey());
  const hasSmtp = Boolean(resolveGmailAuth());

  // 1. Try Gmail REST API (HTTPS) first because it works on Render and sends to ANY email.
  if (hasGmailApi) {
    const apiResult = await sendGmailApiLink({ to, subject, text, html });
    if (apiResult.sent) {
      return apiResult;
    }
  ...
  // 2. Try SMTP (Gmail) next
  if (hasSmtp) {
    const smtpResult = await sendGmailLink({ to, subject, text, html });
    if (smtpResult.sent) {
      return smtpResult;
    }
    // Fallback to Resend if SMTP fails (e.g., if host blocks outbound SMTP)
    if (hasResend) {
